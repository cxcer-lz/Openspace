...
interface TokenRecipient{
  function tokensReceived(address sender,uint256 amount,uint256 tokenId) external returns (bool);
}
...
contract BaseERC20 {
    ...
    function TransferAndCall(address recipient, uint256 amount, uint256 tokenId)external returns (bool) {
      require(amount<=balances[msg.sender],'amount exceeds balance');
        transfer(recipient,amount);
       if (isContract(recipient)) {
            bool rv = TokenRecipient(recipient).tokensReceived(msg.sender, amount,tokenId);
